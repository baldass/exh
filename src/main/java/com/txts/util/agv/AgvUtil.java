...
//		avg.start1();
	...
	/*
	 * public void waitend() throws Exception { byte[] readBuffer = new byte[1024];
	 * inputStream.read(readBuffer); Thread.sleep(10000); //inputStream.reset(); try
	 * { inputStream = serialPort.getInputStream(); // 从线路上读取数据流 int len = 0;
	 * boolean flag = true; while (flag) { System.out.println("等待中...");
	 * Thread.sleep(1000); while ((len = inputStream.read(readBuffer)) > 0) {
	 * Thread.sleep(1000); System.out.println("等待返回"); //test += new
	 * String(readBuffer, 0, len).trim(); if(readBuffer[3]==(byte)01){ break; } }
	 * System.out.println("等待收到数据"); //test += new String(readBuffer, 0,
	 * len).trim(); for (int i = 0; i < len; i++) { byte a = (byte) readBuffer[i];
	 * int ix = a; ix = a & 0xff; System.out.println(ix); }
	 * 
	 * System.out.println("end read."); byte[] b = new byte[] { -29, // 帧头
	 * readBuffer[1], // 对接码 1/2 1, // AGV编号 2, // 主发01 接收02 10, // 指令 2/10 0, // 备用
	 * 暂无数据 -29, // 校验码 - 前面所有字节的亦或码 3// 尾码 };
	 * 
	 * // 校验码(亦或) b[6] = (byte) (b[0] ^ b[1] ^ b[2] ^ b[3] ^ b[4] ^ b[5]);
